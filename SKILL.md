...
## Step 4 — Key `openclaw.json` Settings
...
```bash
# macOS/Linux
node -e "const fs=require('fs'); JSON.parse(fs.readFileSync(process.env.HOME+'/.openclaw/openclaw.json','utf8')); console.log('Config valid ✅')"
```
...
8. **Security hardening** (permissions + config audit)
